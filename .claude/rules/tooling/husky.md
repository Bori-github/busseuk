...
- `.husky/pre-commit` — `pnpm exec lint-staged` 실행. 단, 비대화형 셸(AI 세션·GUI 앱)은
  nvm을 로드하지 않아 `pnpm`이 PATH에 없을 수 있으므로, 먼저 공용 스니펫
  `.claude/hooks/ensure-pnpm.sh`를 source 해 PATH를 보강한다(그래도 없으면 원래 에러로
  실패 — 커밋 게이트는 조용히 스킵하지 않는다. 스니펫 설계: [.claude/hooks/README.md](../../hooks/README.md) 이식성 메모).
