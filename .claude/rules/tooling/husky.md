...
- `.husky/pre-commit` — `pnpm exec lint-staged` 실행. 단, 비대화형 셸(AI 세션·GUI 앱)은
  nvm을 로드하지 않아 `pnpm`이 PATH에 없을 수 있으므로, 스크립트가 먼저 `.nvmrc` 버전의
  nvm 설치 경로를 PATH에 보강한다(그래도 없으면 원래 에러로 실패 — 커밋 게이트는 조용히
  스킵하지 않는다. Claude 훅의 보강 로직과 동일: [.claude/hooks/README.md](../../hooks/README.md) 이식성 메모).
