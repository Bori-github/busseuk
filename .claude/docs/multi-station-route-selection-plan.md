...
적대적 검증에서 도출된 UX 개선안. 필수는 아니며 구현 도중 필요/여유를 보고 반영을 결정한다.

- **전역 5개 상한 카운트 노출**: cap이 정류장 단위가 아니라 전역이라, 한 정류장에서 5개를 채운 뒤 다른 정류장을 열면 모든 체크가 토스트만 뜨고 막힌다("새 정류장인데 왜 안 되지?"). 태그 영역에 `선택 N/5` 카운트를 보여주거나 cap 도달 시 체크박스를 비활성화해 원인을 드러내는 보완.
- **태그에서 도착정보 재확인 동선**: 시트를 닫으면 태그로는 제거만 가능하고 도착정보를 다시 보려면 정류장을 재검색해야 한다. "태그 탭 → 해당 노선의 정류장 시트 재오픈" 동선을 후속으로 검토.
